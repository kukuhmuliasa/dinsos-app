...
<head>
    ...
    <title>{{ $post->title }} - Dinas Sosial Kabupaten Semarang</title>
    ...
    <style>
        ...
        /* --- 1. CSS Animasi Ombak (Identik dengan Welcome) --- */
        ...
        @media (max-width: 768px) { .waves { height: 70px; min-height: 70px; } }

        /* --- 2. Typography & Article Style --- */
        .article-content { color: #374151; }
        .article-content p { margin-bottom: 1.5rem; line-height: 1.8; font-size: 1.125rem; }
        .article-content h2, .article-content h3 { font-weight: 800; color: #0f172a; margin-top: 2.5rem; margin-bottom: 1.25rem; tracking: -0.025em; }
...
<body class="bg-white text-gray-800 selection:bg-yellow-300 selection:text-blue-900 overflow-x-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <ul class="hidden md:flex space-x-8 font-semibold text-sm tracking-wider uppercase">
               <li><a href="/" class="py-2 hover:text-yellow-400 transition-colors relative after:absolute after:bottom-0 after:left-0 after:bg-yellow-400 after:h-0.5 after:w-0 hover:after:w-full after:transition-all after:duration-300">Beranda</a></li>
                ...
                <li><a href="{{ route('posts.index') }}" class="py-2 text-yellow-400 transition-colors relative after:absolute after:bottom-0 after:left-0 after:bg-yellow-400 after:h-0.5 after:w-full">Berita</a></li>
    ...
    <header class="relative min-h-[60vh] md:min-h-[70vh] flex flex-col justify-center items-center text-center text-white bg-cover bg-center overflow-hidden"
            style="background-image: linear-gradient(rgba(10, 30, 70, 0.7), rgba(30, 58, 138, 0.8)), url('{{ $post->image ? asset('storage/' . $post->image) : asset('image/gedung dinsos.jpeg') }}'); background-attachment: fixed;">
        
        <div class="px-4 relative z-30 w-full max-w-5xl animate-fade-up">
            <nav class="flex justify-center space-x-2 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] mb-6 text-blue-200">
                <a href="/" class="hover:text-yellow-400">Beranda</a>
                <span>/</span>
                <a href="{{ route('posts.index') }}" class="hover:text-yellow-400">Berita</a>
            </nav>
            
            <h1 class="text-3xl md:text-6xl font-black leading-tight tracking-tight drop-shadow-2xl mb-8">
                {{ $post->title }}
            </h1>

            <div class="flex items-center justify-center space-x-6 text-[10px] md:text-xs font-bold uppercase tracking-widest text-blue-100">
                <span class="flex items-center bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm">
                    <svg class="w-4 h-4 mr-2 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    {{ $post->created_at->format('d F Y') }}
                </span>
                <span class="flex items-center bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm">
                    <svg class="w-4 h-4 mr-2 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    Admin Dinsos
                </span>
            </div>
        </div>
    ...
    <main class="max-w-4xl mx-auto px-6 py-16 md:py-24">
        <article class="animate-fade-up">
            <div class="text-xl md:text-2xl text-slate-900 leading-relaxed font-semibold italic mb-12 border-l-8 border-yellow-400 pl-8 py-4 bg-slate-50 rounded-r-3xl">
                Dinas Sosial Kabupaten Semarang menyampaikan informasi penting terkait kebijakan dan program pelayanan bagi kesejahteraan masyarakat.
            </div>

            <div class="article-content prose prose-lg prose-blue max-w-none">
                {!! $post->content !!}
            </div>
            ...
            <div class="mt-20 pt-10 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-8">
                ...
                <div class="flex space-x-4">
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}" target="_blank" 
                       class="w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center hover:bg-blue-700 hover:-translate-y-1 transition-all shadow-lg shadow-blue-200">
                    ...
                    <a href="https://api.whatsapp.com/send?text={{ urlencode($post->title) }}%20-%20{{ url()->current() }}" target="_blank"
                       class="w-12 h-12 bg-green-500 text-white rounded-2xl flex items-center justify-center hover:bg-green-600 hover:-translate-y-1 transition-all shadow-lg shadow-green-200">
  ...
  <footer class="bg-blue-950 text-blue-50 pt-20 pb-10 relative overflow-hidden">
    ...
    <script>
        // Navbar Scroll Effect (Identik dengan Welcome)
